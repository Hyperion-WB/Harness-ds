...
## 本仓库增强（相对 upstream 第一版）

- GitHub Actions 三端 release（本地不打包）；壳不自更新。
- Agent 本地 npm 前缀更新。
- 开机自启、全局快捷键、托盘检查 Agent 更新。
- Session iframe 保活与启动路径优先本地前缀（避免冷 `npx`）。
...
│  agent npm-prefix + spawn dsh web                            │
└──────────────────────────────┬──────────────────────────────┘
                               │ http://127.0.0.1:<port>
                               ▼
                     官方 dsh web GUI（iframe 保活）
```
...
2. 否则确保本地 `agent-prefix` 已安装 `@deepseek-ai/dsh`，并用其 `.bin/dsh`。
3. 否则在增强 PATH 上查找 `dsh`。
4. 否则（最后手段）`npx --yes @deepseek-ai/dsh@latest`。

缺少 API Key 时拒绝启动并跳到设置页。Agent 更新走 npm 本地前缀（可自动/手动）；壳本身不自更新，新版本从 GitHub Releases 安装。
